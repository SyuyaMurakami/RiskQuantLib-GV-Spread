...
def downloadData():
    ...
    # 更新期权数据
    df = pd.read_csv(path+os.sep+'Alexdachen-ivix'+os.sep+'options.csv',converters={'DATE':pd.Timestamp})
    ...
    if updateList[-1] != latestUpdateDate:
        ...
        if not updateDF.empty:
            ...
            tmp.to_csv(path + os.sep + 'Alexdachen-ivix' + os.sep + 'options.csv', index=None)
            print("Option Data Download Finished")

    # 更新日期
    df = pd.read_csv(path + os.sep + 'Alexdachen-ivix' + os.sep + 'tradeday.csv',converters={'DateTime':pd.Timestamp})
    ...
    if len(updateList)!=0:
        ...
        df.to_csv(path + os.sep + 'Alexdachen-ivix' + os.sep + 'tradeday.csv', index=None)
    print("Download Trade Day Finish")

    # 更新SHIBOR
    df = pd.read_csv(path + os.sep + 'Alexdachen-ivix' + os.sep + 'shibor.csv',index_col=0)
    ...
    df.to_csv(path + os.sep + 'Alexdachen-ivix' + os.sep + 'shibor.csv')
